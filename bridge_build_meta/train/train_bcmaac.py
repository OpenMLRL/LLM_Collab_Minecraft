...
from LLM_Collab_Minecraft.bridge_build_meta.adapters import BridgeBuildAdapter  # noqa: E402
from LLM_Collab_Minecraft.bridge_build_meta.trainers import BCMAACConfig, BCMAACTrainer  # noqa: E402
...
def _build_bcmaac_config(cfg: Dict[str, Any]) -> BCMAACConfig:
    ...
    return BCMAACConfig(
        ...
        context_learning_rate=float(meta_cfg.get("context_learning_rate", 1e-4)),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train bridge_build with belief-conditioned MAAC.")
    parser.add_argument(
        "--config",
        type=str,
        default=os.path.join(REPO_ROOT, "bridge_build_meta", "configs", "bcmaac_data2_dev.yaml"),
    ...
    if not tokenizer_source:
        raise ValueError("Failed to resolve tokenizer source for bridge_build_meta")
