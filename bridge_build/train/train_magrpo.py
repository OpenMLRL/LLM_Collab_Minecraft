...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train BridgeBuild with MAGRPO (CoMLRL MAGRPOTrainer)")
    parser.add_argument(
        "--config",
        type=str,
        default=os.path.join(REPO_ROOT, "bridge_build", "configs", "bridge_build_magrpo_config.yaml"),
    ...
    configured_num_turns = max(1, int(magrpo_cfg.get("num_turns") or 1))
    ...
    default_batch_item = dict(train_items[0]) if train_items else {}
    ...
    def _lookup_item(prompts: List[str]) -> Dict[str, Any]:
        ...
        return dict(default_batch_item)
    ...
    if isinstance(wandb_cfg, dict) and wandb_cfg.get("enabled", True):
        ...
        try:
            ...
            num_turns_val = 1
        tags = wandb_cfg.get("tags", ["magrpo", dataset_type, f"agents_{num_agents}", f"turns_{num_turns_val}"])
        if not isinstance(tags, list):
            tags = ["magrpo", dataset_type, f"agents_{num_agents}", f"turns_{num_turns_val}"]
        run_name = wandb_cfg.get("name") or wandb_cfg.get("run_name") or f"{dataset_type}-magrpo"
        wandb_config = {
