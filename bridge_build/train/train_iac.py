...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train BridgeBuild with IAC (CoMLRL IACTrainer)")
    parser.add_argument(
        "--config",
        type=str,
        default=os.path.join(REPO_ROOT, "bridge_build", "configs", "bridge_build_iac_config.yaml"),
    ...
    configured_num_turns = max(1, int(iac_cfg.get("num_turns") or 1))
    ...
    if isinstance(wandb_cfg, dict) and wandb_cfg.get("enabled", True):
        ...
        try:
            ...
            num_turns_val = 1
        tags = wandb_cfg.get("tags", ["iac", dataset_type, f"agents_{num_agents}", f"turns_{num_turns_val}"])
        if not isinstance(tags, list):
            tags = ["iac", dataset_type, f"agents_{num_agents}", f"turns_{num_turns_val}"]
        run_name = wandb_cfg.get("name") or wandb_cfg.get("run_name") or f"{dataset_type}-iac"
        wandb_config = {
