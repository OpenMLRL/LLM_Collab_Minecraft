...
# Example SLURM wrapper for str_painter GRPO training (CoMLRL MAGRPOTrainer with num_agents=1/2).
#
# Usage:
#   ACCOUNT=... PARTITION=... ./str_painter/scripts/train_str_painter_grpo_sbatch.sh
#
...
CONFIG_REL="configs/str_painter_config.yaml"
...
if [[ -z "${ACCOUNT}" || -z "${PARTITION}" ]]; then
  ...
  echo "Example:" >&2
  echo "  ACCOUNT=YOUR_ACCOUNT PARTITION=YOUR_PARTITION ./str_painter/scripts/train_str_painter_grpo_sbatch.sh" >&2
  exit 2
fi

# Output dir defaults (YAML placeholders are TODO; set real paths here)
OUTPUT_BASE_DEFAULT="/projects/bevi/${USER}/output/str_painter/[jobid]"
...
sbatch \
  ...
  --mem="${MEM}" \
  --time="${TIME}" \
  --job-name="llm_collab_mc_str_painter_grpo" \
  --wrap="${WRAP_CMD}"

echo "Submitted str_painter GRPO training job with config: ${CONFIG_PATH}"
