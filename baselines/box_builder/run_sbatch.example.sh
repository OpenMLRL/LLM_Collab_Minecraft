...
#   cp baselines/box_builder/run_sbatch.example.sh baselines/box_builder/run_sbatch.sh
#   chmod +x baselines/box_builder/run_sbatch.sh
#   vim baselines/box_builder/run_sbatch.sh
#
# Then submit:
#   ./baselines/box_builder/run_sbatch.sh baselines/box_builder/config.yaml
...
MEM="${MEM:-100g}"
TIME="${TIME:-24:00:00}"
JOB_NAME="${JOB_NAME:-llm_collab_mc_box_builder_mc}"
...
if [[ -z "${ACCOUNT}" || -z "${PARTITION}" || "${ACCOUNT}" == "..." || "${PARTITION}" == "..." || "${ACCOUNT}" == "YOUR_ACCOUNT" || "${PARTITION}" == "YOUR_PARTITION" ]]; then
  ...
  echo "Example:" >&2
  echo "  ACCOUNT=YOUR_ACCOUNT PARTITION=YOUR_PARTITION ./baselines/box_builder/run_sbatch.sh baselines/box_builder/config.yaml" >&2
  exit 2
fi
...
TMP_CONFIG="${SLURM_TMPDIR:-/tmp}/box_builder_config_${SLURM_JOB_ID:-$$}.yaml"
export TMP_CONFIG
...
job_name = os.environ.get("SLURM_JOB_NAME") or "llm_collab_mc_box_builder_mc"
out_fmt = os.environ.get("OUTPUT_FORMAT","auto")

cfg.setdefault("dataset", {})
cfg["dataset"]["json_path"] = str(repo_dir / "dataset/box_builder/one.json")
...
cfg["output"]["path"] = str(repo_dir / "baselines/box_builder/outputs" / out_name)
...
python3 -u baselines/box_builder/main.py --config "${TMP_CONFIG}" ${EXTRA_ARGS}
SBATCH
