...
#   cp baselines/box_builder/run.example.sh baselines/box_builder/run.sh
#   vim baselines/box_builder/run.sh
#
# Then submit:
#   bash baselines/box_builder/run.sh baselines/box_builder/config.yaml
...
if [[ -z "${ACCOUNT}" || -z "${PARTITION}" ]]; then
  ...
  echo "Example:" >&2
  echo "  ACCOUNT=YOUR_ACCOUNT PARTITION=YOUR_PARTITION bash baselines/box_builder/run.sh baselines/box_builder/config.yaml" >&2
  exit 2
fi
...
cd \"${REPO_DIR}\"; \
python3 -u baselines/box_builder/main.py --config \"${CONFIG_PATH}\" ${EXTRA_ARGS}'"
